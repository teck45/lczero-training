...
def do_iteration(net_path, save_path_p, save_path_n, save_path, r=LEARNING_RATE, do_spsa=True):
    ...
    with ThreadPoolExecutor(max_workers=GPUS) as executor:
        ...
        for gpu in range(GPUS):
            cmd = f"""{LC0_PATH} selfplay --player1.weights={save_path_p} --player2.weights={save_path_n} --openings-pgn={BOOK_PATH} --visits={NODES} --games={rounds_per_gpu*2} --mirror-openings=true --temperature=0 --noise-epsilon=0 --fpu-strategy=reduction --fpu-value=0.6 --fpu-strategy-at-root=reduction --fpu-value-at-root=0.6 --cpuct=2.9 --wdl-draw-rate-reference=0.64 --wdl-calibration-elo=3600  --root-has-own-cpuct-params=false --policy-softmax-temp=1.4 --minibatch-size=64 --max-collision-visits=80000 --max-collision-events=917 --moves-left-max-effect=0.03 --moves-left-threshold=0.80 --moves-left-slope=0.0027  --moves-left-quadratic-factor=-0.65 --moves-left-scaled-factor=1.65 --moves-left-constant-factor=0.0 --threads=1 --task-workers=0 --no-share-trees --openings-mode=shuffled  --parallelism=48 --backend=multiplexing --backend-opts=backend=cuda-auto,gpu={gpu}"""
            if SYZYGY:   
