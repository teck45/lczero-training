...
class ApplyAttentionPolicyMap(tf.keras.layers.Layer):
    ...
    def call(self, logits):
        ...
        queen_promotions = tf.constant(1.5, shape=[tf.shape(possible_logits)[0], 22], dtype=logits.dtype)
        rook_promotions = tf.constant(0.5, [tf.shape(possible_logits)[0], 22], dtype=logits.dtype)
        bishop_promotions = tf.constant(-0.5, [tf.shape(possible_logits)[0], 22], dtype=logits.dtype)
        return tf.concat([possible_logits, queen_promotions, rook_promotions, bishop_promotions], axis=1)
